...
ParticlesSystem::ParticlesSystem(std::vector<std::shared_ptr<Particle> > particle_ptrs,
...
void ParticlesSystem::update() {
    ...
    for (auto bond_ptr : bond_ptrs) {
         ...
         if (particle_force_map.find(bond_ptr->getParticle1_ptr()) == particle_force_map.end())
         ...
         f[1] *= -1;
         f[2] *= -1;
         if (particle_force_map.find(bond_ptr->getParticle2_ptr()) == particle_force_map.end())
    ...
    for (auto particle_ptr : particle_ptrs){
            particle_ptr->changeVelocities(particle_force_map[particle_ptr][0] / DEFAULT_MASS * DT,
